...
> **Early preview (`v0.9.5`).** The three core jobs — Library, Discovery,
...
| `ghcr.io/costantinoai/.../:latest` | The image to run. `ghcr.io/...` is the GitHub Container Registry path; `:latest` tracks the newest release on `main`. Pin to `:0.9.5`, `:0.9`, or `:0` on shared servers. |
...
`:0.9.5`, `:0.9`, or `:0`. The lite variant (smaller image, no local
SPECTER2 encoder; see *Two image variants* below) uses `-lite`
suffixes: `:latest-lite`, `:0.9.5-lite`.
...
**`normal`** (the default, `:0.9.5`) includes `torch` + `transformers`,
...
**`lite`** (`:0.9.5-lite`) drops `torch`. Image size is around
