...
| Inbox capture sweep | Scheduler every `INBOX_SWEEP_INTERVAL_MINUTES` (default 5; `0` disables), plus manual via `POST /api/v1/inbox/sweep` or Settings → Plugins → "Check capture now". Polls every active, configured receive integration, resolves each new message and parks the paper at `status='inbox'`. **Self-gating**: returns immediately when no adapter is available, so a token-less install does no work and logs nothing. **Idempotent**: messages are keyed `(channel, external_id)` in `inbox_messages`, so at-least-once delivery — a re-poll, or a crash mid-batch — cannot capture the same paper twice. Network I/O happens strictly outside the write window; the paper row and its ledger row commit together in one `write_section`. Job policy namespace `inbox` (NOTIFICATION, `max_concurrency=1`, `fanout_budget=1`) — overlapping sweeps would re-fetch the same window. |
