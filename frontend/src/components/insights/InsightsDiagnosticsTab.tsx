...
// Tabular trend-point types kept permissive: these shapes come from
// InsightsDiagnosticsTrends and vary per chart. Any additional fields beyond
// those referenced here are simply ignored by Recharts.
type TrendPoint = Record<string, string | number | null | undefined>
type AuthorFollowTrendPoint = { date: string; follows: number }
...
export interface InsightsDiagnosticsTabProps {
  ...
  // Derived slices owned by the parent — passed in so fetch + derive stays
  // with the page's query wiring and the tab stays presentational.
  feedSummary?: InsightsDiagnostics['feed']['summary']
  discoverySummary?: InsightsDiagnostics['discovery']['summary']
  latestFeedRefresh?: InsightsDiagnostics['feed']['recent_refreshes'][number]
  latestDiscoveryRefresh?: InsightsDiagnostics['discovery']['recent_refreshes'][number]
  sourceRequestsTotal: number
  scorecards: NonNullable<InsightsDiagnostics['feed']['scorecards']>
  ...
  ai?: InsightsDiagnostics['ai']
  authors?: InsightsDiagnostics['authors']
  alerts?: InsightsDiagnostics['alerts']
  feedbackLearning?: InsightsDiagnostics['feedback_learning']
  operational?: InsightsDiagnostics['operational']
  coldStartValidation?: InsightsDiagnostics['discovery']['cold_start_topic_validation']
  branchTrends: InsightsDiagnostics['discovery']['branch_trends']

  // Trend series
  trendWindowDays?: number
  feedRefreshTrend: TrendPoint[]
  discoveryRefreshTrend: TrendPoint[]
  recommendationActionTrend: TrendPoint[]
  alertHistoryTrend: TrendPoint[]
  alertHistoryWeeklyTrend: TrendPoint[]
  authorFollowTrend: AuthorFollowTrendPoint[]
  feedbackLearningTrend: TrendPoint[]

  // Drilldown persistence + handlers
...
// ── Local primitives (Diagnostics-scoped refinement) ------------------------
//
// Diagnostics-only helpers. MetricTile + SectionHeader are shared primitives
// in `components/shared` — imported above.
...
function CalloutWarning({ children }: { children: React.ReactNode }) {
  // Tone comes from `variant="warning"` on the Alert primitive — do not
  // add inline `border-amber-*` / `bg-amber-*` / `text-amber-*` classes
  // here or the callout will drift from every other warning surface.
  return (
...
/** One row in the "Recent refreshes" card. Collapses the Feed + Discovery
 *  halves which used near-identical markup.
 */
...
export function InsightsDiagnosticsTab(props: InsightsDiagnosticsTabProps) {
  const {
    loading,
    error,
    diagnostics,
  ...
  feedbackLearning,
    ...
    trendWindowDays,
    feedRefreshTrend,
    discoveryRefreshTrend,
    recommendationActionTrend,
    alertHistoryTrend,
    alertHistoryWeeklyTrend,
    authorFollowTrend,
  feedbackLearningTrend,
    savedDrilldowns,
    onSaveDrilldown,
  ...
  const branchSourceBadges = useMemo(
    () =>
      (diagnostics?.discovery.branch_quality ?? []).map((branch) => ({
        key: branch.branch_id ?? branch.branch_label,
        mix: branch.source_mix,
      })),
    [diagnostics?.discovery.branch_quality],
  )
  ...
  if (error || !diagnostics) {
    return <ErrorState message="Failed to load diagnostics." />
  }

  const windowDays = trendWindowDays ?? 30
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          {automationOpportunities.length === 0 ? (
            ...
            <div className="grid gap-3 xl:grid-cols-2">
              {automationOpportunities.map((template) => (
                <div
                  key={template.key}
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr,0.8fr]">
        <Card>
          ...
          <CardContent className="space-y-4">
            {!ai ? (
              <EmptyState title="No AI diagnostics available yet" />
            ) : (
              <>
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  <MetricTile
                    label="Coverage"
                    value={`${ai.summary.embedding_coverage_pct}%`}
                    hint={`${ai.summary.up_to_date_embeddings}/${ai.summary.total_papers} up to date`}
                  />
                  <MetricTile
                    label="Compressed"
                    value={`${Math.round((ai.summary.compressed_similarity_rate ?? 0) * 100)}%`}
                    hint="Recent recs in weak semantic ranges"
                  />
                  <MetricTile
                    label="Hybrid use"
                    value={`${Math.round((ai.summary.hybrid_text_rate ?? 0) * 100)}%`}
                ...
                <div className="grid gap-4 xl:grid-cols-2">
                  <div className="rounded-sm border border-[var(--color-border)] p-4">
                    ...
                    <div className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2">
                      <span>Avg text similarity: {ai.summary.avg_text_similarity}</span>
                      <span>Avg semantic raw: {ai.summary.avg_semantic_raw}</span>
                      <span>Avg semantic support: {ai.summary.avg_semantic_support_raw}</span>
                      <span>Avg lexical term raw: {ai.summary.avg_lexical_term_raw}</span>
                      <span>
                        Candidate embeddings ready:{' '}
                        {Math.round((ai.summary.embedding_candidate_ready_rate ?? 0) * 100)}%
                      </span>
                      <span>
                        Low-similarity rate:{' '}
                        {Math.round((ai.summary.low_similarity_rate ?? 0) * 100)}%
                      </span>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Badge variant="outline">
                        semantic {Math.round((ai.summary.semantic_only_rate ?? 0) * 100)}%
                      </Badge>
                      <Badge variant="outline">
                        lexical {Math.round((ai.summary.lexical_only_rate ?? 0) * 100)}%
                      </Badge>
                      <Badge variant="outline">
                        hybrid {Math.round((ai.summary.hybrid_text_rate ?? 0) * 100)}%
                      </Badge>
                      <Badge variant="outline">
                        dims {ai.summary.dominant_embedding_dimension || 'n/a'} /{' '}
                        {ai.summary.embedding_dimension_variants} variants
                      </Badge>
                    </div>
                  </div>
          ...
          <CardContent className="space-y-3">
            {!ai || ai.recommendations.length === 0 ? (
              ...
              ai.recommendations.map((item) => (
                <div key={item.id} className="rounded-sm border border-[var(--color-border)] p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <MetricTile label="Tracked authors" value={authors?.summary.tracked_authors ?? 0} />
              <MetricTile
                label="Provenance-only"
                value={authors?.summary.provenance_only_authors ?? 0}
              />
              <MetricTile
                label="Ready monitors"
                value={authors?.summary.ready_tracked ?? 0}
                tone="success"
              />
              <MetricTile
                label="Bridge gaps"
                value={authors?.summary.bridge_gap_count ?? 0}
                tone={(authors?.summary.bridge_gap_count ?? 0) > 0 ? 'warning' : 'neutral'}
              />
              <MetricTile
                label="Background corpus papers"
                value={authors?.summary.background_corpus_papers ?? 0}
              />
            </div>
            {(authors?.degraded ?? []).length > 0 ? (
              <div className="space-y-3">
                {authors?.degraded.map((author) => (
                  <div
            ...
            {(authors?.suggestions ?? []).length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {(authors?.suggestions ?? []).slice(0, 4).map((suggestion) => (
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <MetricTile label="Enabled alerts" value={alerts?.summary.enabled_alerts ?? 0} />
              <MetricTile
                label="Active in 30d"
                value={alerts?.summary.active_alerts_30d ?? 0}
              />
              <MetricTile
                label="Sent runs"
                value={alerts?.summary.sent_runs_30d ?? 0}
                tone="success"
              />
              <MetricTile
                label="Avg papers / sent"
                value={alerts?.summary.avg_papers_per_sent ?? 0}
                tone="warning"
              />
            </div>
            {alerts?.long_horizon?.summary ? (
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <StatusBadge
                    tone={scoreStatusTone(
                      alertUsefulnessTone(alerts.long_horizon.summary.usefulness_score ?? 0),
                    )}
                  >
                    {alerts.long_horizon.summary.usefulness_score}/100
                  </StatusBadge>
                </div>
                <div className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-3">
                  <span>90d sent: {alerts.long_horizon.summary.sent_runs}</span>
                  <span>90d failed: {alerts.long_horizon.summary.failed_runs}</span>
                  <span>
                    Delta vs 30d:{' '}
                    {alerts.long_horizon.summary.delta_vs_30d >= 0 ? '+' : ''}
                    {alerts.long_horizon.summary.delta_vs_30d.toFixed(0)}
                  </span>
                </div>
              </div>
            ) : null}
            {(alerts?.top_alerts ?? []).length > 0 ? (
              <div className="space-y-3">
                {alerts?.top_alerts.map((alert) => (
                  <div
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {alert.sent_runs} sent · {alert.empty_runs} empty · {alert.failed_runs}{' '}
                          failed
                        </p>
                      </div>
                      <StatusBadge
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <MetricTile
                label="Week interactions"
                value={feedbackLearning?.summary.week_interactions ?? 0}
              />
              <MetricTile label="Streak days" value={feedbackLearning?.summary.streak_days ?? 0} />
              <MetricTile
                label="Positive topics"
                value={feedbackLearning?.summary.topic_coverage ?? 0}
              />
              <MetricTile
                label="Sources touched"
                value={feedbackLearning?.summary.source_diversity_7d ?? 0}
              />
              <MetricTile
                label="Background papers used"
                value={feedbackLearning?.summary.background_corpus_papers ?? 0}
              />
              <MetricTile
                label="Background authors used"
                value={feedbackLearning?.summary.background_corpus_authors ?? 0}
              />
            </div>
            {(feedbackLearning?.top_topics ?? []).length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-medium text-slate-700">Top topics</p>
                <div className="flex flex-wrap gap-2">
                  {(feedbackLearning?.top_topics ?? []).slice(0, 5).map((topic, index) => (
            ...
            {(feedbackLearning?.top_authors ?? []).length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-medium text-slate-700">Top authors</p>
                <div className="flex flex-wrap gap-2">
                  {(feedbackLearning?.top_authors ?? []).slice(0, 5).map((author, index) => (
            ...
            {(feedbackLearning?.next_actions ?? []).length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-medium text-slate-700">Next actions</p>
                <ul className="space-y-1 text-sm text-slate-500">
                  {(feedbackLearning?.next_actions ?? []).map((item) => (
                    <li key={item}>· {item}</li>
                  ))}
                </ul>
              </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <MetricTile
                ...
                value={operational?.summary.critical_count ?? 0}
                tone={(operational?.summary.critical_count ?? 0) > 0 ? 'critical' : 'neutral'}
              />
              <MetricTile
                label="Warnings"
                value={operational?.summary.warning_count ?? 0}
                tone={(operational?.summary.warning_count ?? 0) > 0 ? 'warning' : 'neutral'}
              />
              <MetricTile
                label="Unhealthy plugins"
                value={operational?.summary.unhealthy_plugins ?? 0}
              />
              <MetricTile
                label="Failed ops 24h"
                value={operational?.summary.recent_failed_operations_24h ?? 0}
              />
            </div>
            {(operational?.states ?? []).length > 0 ? (
              <div className="space-y-3">
                {operational?.states.slice(0, 5).map((state) => (
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          />
          <CardContent>
            {feedRefreshTrend.length === 0 ? (
              <EmptyState title="No Feed refresh trend yet" />
            ) : (
              <ResponsiveContainer width="100%" height={220}>
                <ComposedChart data={feedRefreshTrend}>
        ...
        <Card>
          ...
          />
          <CardContent>
            {discoveryRefreshTrend.length === 0 ? (
              <EmptyState title="No Discovery refresh trend yet" />
            ) : (
              <ResponsiveContainer width="100%" height={220}>
                <ComposedChart data={discoveryRefreshTrend}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E9DCBC" />
                  <XAxis dataKey="date" tick={{ fontSize: 11 }} interval="preserveStartEnd" minTickGap={30} />
                  <YAxis yAxisId="left" tick={{ fontSize: 11 }} allowDecimals={false} />
                  <YAxis
        ...
        <Card>
          ...
          />
          <CardContent>
            {recommendationActionTrend.length === 0 ? (
              <EmptyState title="No recommendation-action trend yet" />
            ) : (
              <ResponsiveContainer width="100%" height={220}>
                <ComposedChart data={recommendationActionTrend}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E9DCBC" />
                  <XAxis dataKey="date" tick={{ fontSize: 11 }} interval="preserveStartEnd" minTickGap={30} />
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          />
          <CardContent>
            {authorFollowTrend.length === 0 ? (
              <EmptyState title="No followed-author trend yet" />
            ) : (
              <ResponsiveContainer width="100%" height={220}>
                <ComposedChart data={authorFollowTrend}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E9DCBC" />
                  <XAxis dataKey="date" tick={{ fontSize: 11 }} interval="preserveStartEnd" minTickGap={30} />
                  <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                  <Tooltip {...tooltipStyle} />
        ...
        <Card>
          ...
          />
          <CardContent>
            {feedbackLearningTrend.length === 0 ? (
              <EmptyState title="No feedback-learning trend yet" />
            ) : (
              <ResponsiveContainer width="100%" height={220}>
                <ComposedChart data={feedbackLearningTrend}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E9DCBC" />
                  <XAxis dataKey="date" tick={{ fontSize: 11 }} interval="preserveStartEnd" minTickGap={30} />
                  <YAxis yAxisId="left" tick={{ fontSize: 11 }} allowDecimals={false} />
                  <YAxis
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          />
          <CardContent>
            {(diagnostics.feed.monitors ?? []).length === 0 ? (
              <EmptyState title="No feed monitors available" />
            ) : (
              <div className="space-y-3">
                {diagnostics.feed.monitors.map((monitor) => (
                  <div key={monitor.id} className="rounded-sm border border-[var(--color-border)] p-3">
        ...
        <Card>
          ...
          />
          <CardContent>
            {(diagnostics.discovery.source_diagnostics ?? []).length === 0 ? (
              <EmptyState title="No source diagnostics available yet" />
            ) : (
              <div className="space-y-3">
                {diagnostics.discovery.source_diagnostics.map((source) => (
                  <div key={source.source} className="rounded-sm border border-[var(--color-border)] p-3">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                ...
                <div className="rounded-sm border border-[var(--color-border)] p-3">
                  ...
                  <div className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2">
                    <span>Refreshes: {diagnostics.discovery.openalex_usage.refreshes}</span>
                    <span>Requests: {diagnostics.discovery.openalex_usage.request_count}</span>
                    <span>Retries: {diagnostics.discovery.openalex_usage.retry_count}</span>
                    <span>
                      Saved by cache: {diagnostics.discovery.openalex_usage.calls_saved_by_cache}
                    </span>
                  </div>
                </div>
              </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          />
          <CardContent>
            {(diagnostics.discovery.source_quality ?? []).length === 0 ? (
              <EmptyState title="No discovery source quality data available" />
            ) : (
              <div className="space-y-3">
                {diagnostics.discovery.source_quality.map((source) => (
                  <div
        ...
        <Card>
          ...
          />
          <CardContent>
            {coldStartValidation && coldStartValidation.total_runs > 0 ? (
              <div className="mb-4 rounded-sm border border-[var(--color-border)] bg-parchment-50 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Badge variant="outline">
                    {coldStartValidation.validated_runs}/{coldStartValidation.total_runs} validated
                  </Badge>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  {Object.entries(coldStartValidation.state_counts ?? {}).map(([state, count]) => (
            ...
            {(diagnostics.discovery.branch_quality ?? []).length === 0 ? (
        ...
        <CardContent>
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3">
              ...
              {(diagnostics.feed.recent_refreshes ?? []).length === 0 ? (
                <p className="text-sm text-slate-400">No recent Feed refreshes.</p>
              ) : (
                diagnostics.feed.recent_refreshes.map((refresh) => (
                  <RefreshEntryRow
                    key={refresh.job_id}
            ...
            <div className="space-y-3">
              ...
              {(diagnostics.discovery.recent_refreshes ?? []).length === 0 ? (
                <p className="text-sm text-slate-400">No recent Discovery refreshes.</p>
              ) : (
                diagnostics.discovery.recent_refreshes.map((refresh) => (
                  <RefreshEntryRow
                    key={refresh.job_id}
                    status={refresh.status}
                    finishedAt={refresh.finished_at}
